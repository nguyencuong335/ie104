...
document.addEventListener("DOMContentLoaded", () => {
                        ...
                        try {
                            ...
                            const finalize = (dataUrl) => {
                                try {
                                    ...
                                    // immediate UI update
                                    const coverEl =
                                        card.querySelector(".my-pl-cover");
                                    if (coverEl) {
    ...
    function wireSidebarPlaylists() {
        try {
            ...
            items.forEach((it, i) => {
                if (!it.dataset.plId)
                    it.dataset.plId = demoIds[i] || `pl_${i + 1}`;
                it.addEventListener("click", () => {
                    const id = it.dataset.plId;
                    try {
                        go(`./playlist.html?id=${encodeURIComponent(id)}`);
                    } catch {
                        window.location.href = `./playlist.html?id=${encodeURIComponent(
                            id
                        )}`;
          ...
          `;
            ...
            addBtn.addEventListener("click", () => {
                try {
                    ...
                    if (
                        Array.isArray(lists) &&
                        lists.length >= MAX_USER_PLAYLISTS
                    ) {
                        try {
                            showToast(
    ...
    (function syncSidebarPlaylists() {
        try {
            ...
            items.forEach((it, i) => {
                ...
                if (name) {
                    const t = String(pl.name || name.textContent);
                    name.textContent =
    ...
    window.addEventListener("playlists:changed", () => {
        ...
        `;
                ...
                if (cov2) {
                    cov2.style.backgroundImage = `url('${
                        pl.cover ||
                        "./assets/imgs/danh_sach_da_tao/anh_playlist_1.jpg"
                    }')`;
    ...
    document.addEventListener(
        ...
        (e) => {
            ...
            if (right && (right.contains(e.target) || e.target === volIcon))
                return;
            hideMobileVolume();
        },
        true
